...
<head>
    ...
    <link type="text/css" href="/test/css/slider.css" rel="stylesheet">
    <script type="text/javascript" src="/test/js/jquery-2.1.3.min.js"></script>
    <script type="text/javascript" src="/test/js/jquery-ui.js"></script>
    <script type="text/javascript" src="/test/js/resultPage.js"></script>
    <link href="/test/css/jquery-ui.css" type="text/css" rel="stylesheet">
</head>
<body>
  <div id="timeSlider">
     <div> <%@include file="/goDepartSlider.jsp"%></div>
     <div> <%@include file="/goArriveSlider.jsp"%></div>
    <c:if test="${type!='oneWay'}">
      <hr>
      <div> <%@include file="/comeDepartSlider.jsp"%></div>
      <div> <%@include file="/comeArriveSlider.jsp"%></div>
    </c:if>
     <input type="button" id="updateTime" value="Update" onclick="changeTime()">
  </div>
  <div id="main">
    <%--顶部条件选择区域--%>
    <div id="conditions">
      ...
      <div id="changeCon">
        <table id="originalInfo">
          ...
          <tr>
            ...
            <td><input id="changeBtn" type="button" onclick="goBack()" value="Change"></td>
          </tr>
          </tbody>
        </table>
      </div>
    ...
    <%--搜索结果显示--%>
      <c:if test="${empty(deZone)}">
        <c:set value="${my:formater(tripInfo.departCode)}" var="deZone" scope="session"/>
        <c:set value="${my:formater(tripInfo.arriveCode)}" var="arZone" scope="session"/>
      </c:if>
      <c:choose>
        ...
        <c:otherwise>
          <c:forEach var="showItem" items="${showInfo}">
            <c:choose>
              ...
              <%--出发单程,返回转机--%>
              <c:when test="${showItem.value['0'].getClass().name=='beans.Flight' && showItem.value['1'].getClass().name!='beans.Flight'}">
                <%@include file="/WEB-INF/jsp/roundMix1.jsp"%>
              </c:when>
              <%--出发转机,返回单程--%>
              <c:when test="${showItem.value['0'].getClass().name!='beans.Flight' && showItem.value['1'].getClass().name=='beans.Flight'}">
                <%@include file="/WEB-INF/jsp/roundMix2.jsp"%>
              </c:when>
              <%--往返航班并且有转机--%>
              <c:otherwise>
