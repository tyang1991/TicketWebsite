...
function checkReturnDate(input){
    var flag;
    var spanNode = $(input).parent().children("span");
    if(!checkDate(input)){
        flag = false;
    ...
    var value = $(input).val();
    var backDate = new Date(Date.parse(value));
    var go = $("#calendar1").val();
    var goDate = new Date(Date.parse(go));
    //alert(goDate.getTime()+"   "+backDate.getTime());
    if(goDate.getTime()>backDate.getTime()){
        $(input).addClass("invalideInput");
        spanNode.text("Invalid date").css("color", "red");
        flag=false;
        return flag;
...
function checkForm(){
    ...
    if(checkEmpty(deId) && checkEmpty(arId) && checkDate(calId1) && checkReturnDate(calId2) && checkNum(num)){
        return true;
    }
    return false;
}