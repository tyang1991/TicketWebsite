...
public class ConnectDaoImpl implements ConnectDao {
    public List<SearchKey> airportSearchInfo(Document document, String airportType){
        ...
        for ( Iterator i = root.elementIterator( "Flight" ); i.hasNext(); ) {
            ...
            String planeType = element.attribute("Airplane").getValue();
            //获取该类型飞机的作为总数
            int totalCoachSeat = PlaneSeat.getSeatNum(planeType,ConstantVariable.COACH);
            int totalFirstSeat = PlaneSeat.getSeatNum(planeType,ConstantVariable.FIRST);

            int firstCurrent = Integer.parseInt(element.element("Seating").element("FirstClass").getText());
            int firstLeft = totalFirstSeat-firstCurrent;

            int coachCurrent = Integer.parseInt(element.element("Seating").element("Coach").getText());
            int coachLeft = totalCoachSeat - coachCurrent;
